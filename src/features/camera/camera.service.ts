...
export class CameraService {
  ...
  public async findCameraType() {
    //取得所有
    // 查询 camera_type 表
    const cameraTypes = await this.cameraTypeRepository
      .query(
        `
    ...
    `,
      ...
      .then((items) =>
        items.map((item) => ({ ...item, mainProductType: '攝影機' })),
      );

    // 查询 host_type 表
    const hostTypes = await this.cameraTypeRepository
      .query(
        `
    ...
    `,
      ...
      .then((items) =>
        items.map((item) => ({ ...item, mainProductType: '主機' })),
      );

    // 查询 other_type 表
    const otherTypes = await this.cameraTypeRepository
      .query(
        `
    ...
    `,
      ...
      .then((items) =>
        items.map((item) => ({ ...item, mainProductType: '其他產品' })),
      );

    const reslut = [...cameraTypes, ...hostTypes, ...otherTypes].map(
      (item) => ({
